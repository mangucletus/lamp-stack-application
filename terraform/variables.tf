...
# Root password for the MySQL database (should be stored securely)
variable "db_password" {
  description = "MySQL root password"  # Purpose of the password
  type        = string                 # Must be a string
  sensitive   = true                   # Hides from CLI/UI logs
  default     = "SecurePass123!"       # Default password (ideally override in production)
}
